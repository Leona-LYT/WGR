...
def main():
    ...
    for k in range(args.reps):
        ...
        trained_G, trained_D = train_WGR_fnn(D=D_net, G=G_net, D_solver=D_solver, G_solver=G_solver, loader_train = loader_train, 
                                             ...
                                             lambda_w=1, lambda_l = 0, save_name = ' cWGAN', 
    ...
    test_G_mean_sd_csv.to_csv("./test_cWGAN_mean_sd_"+str(args.model)+"_d"+str(args.Xdim)+".csv")
    test_G_quantile_csv.to_csv("./test_cWGAN_quantile_"+str(args.model)+"_d"+str(args.Xdim)+".csv")
