        ...
        if y is not None:
            with pyro.plate("data", x.shape[0]):
                if self.likelihood == 'gaussian':
                    if self.out_dim > 1:
                        sigma = pyro.sample("sigma", dist.Gamma(1.0, 1.0).expand([self.out_dim]).to_event(1))
                    ...
                    if self.out_dim > 1:
                        obs = pyro.sample("obs", dist.Bernoulli(probs).to_event(1), obs=y)
                    else:
                    ...
                    if self.out_dim > 1:
                        logits = mu
                        obs = pyro.sample("obs", dist.OneHotCategorical(logits=logits), obs=y)
                    else:
                        # 单个类别标签
