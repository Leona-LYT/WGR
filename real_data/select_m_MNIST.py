...
from utils.basic_utils import setup_seed, sample_noise, l2_loss
...
candidate_set = set([i for i in range(144, 221, 4)])
sorted_list = sorted(candidate_set)  # Convert set to a sorted list

# Define the m selection function for MNIST dataset
# This implementation differs from others because computing all images simultaneously is computationally expensive.
def mnist_select_m(G, Xdim, noise_dim, train_size):
    with torch.no_grad():
        L2_value = torch.zeros([len(train_loader)])
        for batch_idx, (x,y,label) in enumerate(train_loader):
            ...
            L2_value[batch_idx] = l2_loss(output,y)

        value_m = torch.mean(L2_value) + (Xdim + noise_dim)*np.log(train_size)/train_size
    return value_m 
...
for k in range(args.m_set):
    ...
    G_net = generator_fnn(Xdim=args.Xdim, Ydim=args.Ydim, noise_dim=sorted_list[k], network_type = 'mnist', hidden_dims = [1024, 1024, 512], final_activation = 'tanh')
    D_net = discriminator_fnn(input_dim=784, network_type = 'mnist',  hidden_dims = [256, 256 ])
    ...
    trained_G, trained_D = train_WGR_image(D=D_net,G=G_net, D_solver=D_solver,G_solver=G_solver,Xdim=args.Xdim, Ydim=args.Ydim, 
                                           noise_dim=sorted_list[k], loader_data=train_loader,  loader_val=val_loader, 
                                           batch_size = args.train_batch, eg_x =eg_x,  eg_label=eg_label, save_path = './MNIST/',
                                           selected_indices=selected_indices, lambda_w=0.9, lambda_l=0.1, num_epochs=200)    
    
    score_m = mnist_select_m(G=trained_G, Xdim=args.Xdim, noise_dim=sorted_list[k], train_size=args.train)

    G_m_score.append(score_m)

    G_m_score_csv = pd.DataFrame(G_m_score)

    G_m_score_csv.to_csv("./G_m_score_MNIST_d"+str(args.Xdim)+"_m_select.csv")

    eg_eta =  sample_noise(args.train_batch, dim=sorted_list[k]).to(device)
