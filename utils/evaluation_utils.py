...
# =============================================================================
def L1L2_MSE_mean_sd_G(G, loader_dataset, noise_dim, Xdim, test_size, batch_size, distribution='gaussian', J_t_size=50, model_type="M1", Ydim=1, is_multivariate=False):
    """
...
# =============================================================================
def MSE_quantile_G_uniY(G, loader_dataset,  noise_dim, Xdim, test_size, batch_size,distribution='gaussian', J_t_size=500, model_type="M1" ):
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx, (x, y) in enumerate(loader_dataset):
            ...
            outputs = torch.stack([
                G(torch.cat([x, sample_noise(x.size(0), noise_dim, distribution=distribution , mu=mu, cov=cov, a=a, b=b, loc=loc, scale=scale)], dim=1)).view(x.size(0))
                for _ in range(J_t_size)
            ])
...
def MSE_quantile_G_multiY(G, loader_dataset,  Ydim, Xdim, noise_dim, batch_size, test_size, model_type,distribution='gaussian', J_t_size=500):
    """
...
def eva_G_UniY(G, loader_data, noise_dim, test_size, batch_size,distribution='gaussian', J_t_size=50):
    """
...
def eva_G_MultiY(G, loader_data, Ydim, noise_dim, batch_size,distribution='gaussian', J_t_size=50):
    """
    Evaluate a generator model on real data analysis.
